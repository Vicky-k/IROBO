...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("angry","happy")
  ...
  def classifyImage(sc: SparkContext, path: String): Double = {
    ...
    val p = nbModel.predict(histogram)
    println(s"Predicting test image : " + p)

    p
  }

  def getJavaFXImage(rawPixels:Array[Byte]) = {
    val out = new ByteArrayOutputStream
    try {
      ImageIO.write(createBufferedImage(rawPixels, 400, 400).asInstanceOf[RenderedImage], "png", out)
      out.flush
    } catch {
  ...
  def main(args: Array[String]) {
   ...
   // scnn.receiveImage();
    ...
    testImagesArray.foreach(f => {
      ...
      println(s"Predicting test image : " + " as " + IMAGE_CATEGORIES(predictedClass.toInt))
      // println(s"Predicting test image : " + cat + " as ")
      scnn.initialize2(IMAGE_CATEGORIES(predictedClass.toInt));
